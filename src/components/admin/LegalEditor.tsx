...
export default function LegalEditor() {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-32 text-adm-ink-faint bg-adm-surface rounded-md border border-adm-border">
            <FileText className="w-10 h-10 animate-pulse mb-6 text-adm-ink-faint" />
    ...
    return (
        <div className="space-y-6 pb-32">
            {/* Tabs */}
            <div className="flex items-center justify-between bg-adm-surface p-4 rounded-lg border border-adm-border shadow-sm">
                <div role="tablist" aria-label="Documentos legais" className="flex p-1 bg-adm-elev rounded-md w-fit border border-adm-border">
                    <button
                        role="tab"
                        ...
                        className={`px-4 py-2 min-h-[44px] rounded text-xs font-bold transition-all ${activeTab === 'privacy' ? 'bg-adm-surface text-adm-ink shadow-sm border border-adm-border' : 'text-adm-ink-muted hover:text-adm-ink'}`}
                    >
                    ...
                    <button
                        ...
                        className={`px-4 py-2 min-h-[44px] rounded text-xs font-bold transition-all ${activeTab === 'terms' ? 'bg-adm-surface text-adm-ink shadow-sm border border-adm-border' : 'text-adm-ink-muted hover:text-adm-ink'}`}
                    >
                        Termos de Uso
                    </button>
                </div>
                <button onClick={() => handleSave(activeTab)} disabled={saving}
                    className="bg-adm-primary hover:bg-adm-primary disabled:opacity-50 text-white px-5 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all">
            ...
            {currentData ? (
                ...
                >
                    {currentData.content.map((section, idx) => (
                        <div key={idx} className="bg-adm-surface p-6 rounded-lg border border-adm-border shadow-sm group">
                            <div className="flex items-center justify-between mb-4 border-b border-adm-border pb-4">
                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                    <span aria-hidden="true" className="w-8 h-8 rounded-md bg-adm-elev flex items-center justify-center text-adm-ink-muted font-bold text-xs shrink-0">#{idx + 1}</span>
                                    <input
                                        type="text"
                                        value={section.title}
                                        onChange={(e) => updateSection(activeTab, idx, 'title', e.target.value)}
                                        aria-label={`Título da seção ${idx + 1}`}
                                        className="text-sm font-bold text-adm-ink bg-transparent border-none focus:ring-0 w-full focus:outline-none"
                                        placeholder="Título da Seção"
                                    />
                                </div>
                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity shrink-0">
                                    <button onClick={() => moveSection(activeTab, idx, 'up')} disabled={idx === 0} aria-label={`Mover seção ${idx + 1} para cima`} className="p-2 min-h-[44px] min-w-[44px] text-adm-ink-faint hover:text-adm-primary hover:bg-adm-primary-soft rounded transition-colors disabled:opacity-30"><ChevronUp className="w-4 h-4" aria-hidden="true" /></button>
                                    <button onClick={() => moveSection(activeTab, idx, 'down')} disabled={idx === currentData.content.length - 1} aria-label={`Mover seção ${idx + 1} para baixo`} className="p-2 min-h-[44px] min-w-[44px] text-adm-ink-faint hover:text-adm-primary hover:bg-adm-primary-soft rounded transition-colors disabled:opacity-30"><ChevronDown className="w-4 h-4" aria-hidden="true" /></button>
                                    <button onClick={() => removeSection(activeTab, idx)} aria-label={`Excluir seção ${idx + 1}: ${section.title}`} className="p-2 min-h-[44px] min-w-[44px] text-adm-ink-faint hover:text-red-600 hover:bg-red-50 rounded ml-2 transition-colors"><Trash2 className="w-4 h-4" aria-hidden="true" /></button>
                                </div>
                            </div>
                            <textarea value={section.text} onChange={(e) => updateSection(activeTab, idx, 'text', e.target.value)} rows={6}
                                className="w-full bg-adm-surface border border-adm-border rounded-md px-4 py-3 text-adm-ink text-base leading-relaxed focus:outline-none focus:border-adm-primary/80 focus:ring-1 focus:ring-adm-primary/20 transition-colors resize-none shadow-sm"
                                placeholder="Escreva o texto jurídico aqui..." />
                        </div>
                    ))}
                    <button onClick={() => addSection(activeTab)} className="w-full py-8 border-2 border-dashed border-adm-border rounded-md text-adm-ink-muted hover:text-adm-primary hover:border-adm-primary hover:bg-adm-primary-soft transition-all font-bold flex flex-col items-center justify-center gap-2 text-xs uppercase">
