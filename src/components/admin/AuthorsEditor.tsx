...
export default function AuthorsEditor() {
    ...
    return (
        <div className="space-y-8 pb-32">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-adm-surface/80 backdrop-blur-xl p-5 px-8 rounded-lg border border-adm-border shadow-xl shadow-slate-200/50 sticky top-0 z-40">
                <div>
                    ...
                    <p className="text-xs font-bold text-adm-ink-muted uppercase tracking-widest mt-1 flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full border-2 border-adm-primary"></span>
                        {authors.length} Perfis Cadastrados
                    </p>
                </div>
                <div className="flex items-center gap-3 w-full sm:w-auto">
                    {saving && <div className="flex items-center gap-2 text-adm-primary bg-adm-elev px-4 py-2 rounded-lg text-sm font-bold mr-2"><Loader2 className="w-4 h-4 animate-spin" /> Sincronizando...</div>}
                    <button onClick={() => { setTempAuthor({ id: `author-${Date.now()}`, name: '', role: '', avatar: '', bio: '', social: { twitter: '', instagram: '', linkedin: '', website: '' } }); setEditingIndex(null); setIsModalOpen(true); }} disabled={saving}
                        className="w-full sm:w-auto bg-adm-primary hover:bg-adm-primary disabled:bg-slate-300 disabled:cursor-not-allowed text-white px-6 py-3 rounded-md font-bold flex items-center justify-center gap-2 shadow-lg shadow-none hover:-translate-y-0.5 transition-all">
            ...
            {authors.length === 0 ? (
                <div className="bg-adm-elev border-2 border-dashed border-adm-border rounded-lg p-16 flex flex-col items-center justify-center text-center w-full mt-6">
                    <div className="w-20 h-20 bg-adm-surface rounded-full flex items-center justify-center text-adm-ink-faint mb-4 shadow-sm"><Users className="w-10 h-10" /></div>
                    <h3 className="text-xl font-bold text-adm-ink mb-2">Sua equipe está vazia!</h3>
                    ...
                    <button onClick={() => { setTempAuthor({ id: `author-${Date.now()}`, name: '', role: '', avatar: '', bio: '', social: { twitter: '', instagram: '', linkedin: '', website: '' } }); setEditingIndex(null); setIsModalOpen(true); }}
                        className="bg-adm-primary text-white font-bold px-8 py-3 rounded-md shadow-md hover:bg-adm-primary transition-colors inline-flex items-center gap-2">
                        <Plus className="w-5 h-5" aria-hidden="true" /> Adicionar Primeiro Autor
                    </button>
                </div>
            ) : (
                <div className="bg-adm-surface rounded-md border border-adm-border overflow-hidden shadow-sm mt-6">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-adm-elev border-b border-adm-border">
                                ...
                                {authors.map((author, idx) => (
                                    <tr key={author.id || idx} className="hover:bg-adm-elev transition-colors group">
                                        <td className="py-4 px-6 align-middle">
                                            <div className="w-12 h-12 rounded-full overflow-hidden shadow-sm flex items-center justify-center shrink-0 bg-adm-surface border-2 border-adm-border">
                                                {author.avatar ? <img src={author.avatar} alt="Avatar" className="w-full h-full object-cover" /> : <ImageIcon className="w-5 h-5 text-adm-ink-faint" />}
                                            </div>
                                        </td>
                                        <td className="py-4 px-6 align-middle">
                                            <p className="font-bold text-adm-ink text-sm mb-1">{author.name || 'Sem nome'}</p>
                                            <p className="text-xs font-bold text-adm-primary">{author.role || 'Sem cargo'}</p>
                                        </td>
                                        <td className="py-4 px-6 align-middle">
                                        ...
                                        <td className="py-4 px-6 align-middle text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <button onClick={() => { setTempAuthor({ ...author }); setEditingIndex(idx); setIsModalOpen(true); }}
                                                    className="w-8 h-8 bg-adm-elev text-adm-ink-muted rounded-lg inline-flex items-center justify-center hover:bg-adm-elev hover:text-adm-ink transition-colors">
            ...
            {isModalOpen && tempAuthor && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-adm-ink/60 backdrop-blur-sm" aria-hidden="true">
                    <div
                        ref={modalRef}
                        role="dialog"
                        aria-modal="true"
                        ...
                        aria-hidden="false"
                        className="bg-adm-surface rounded-lg w-full max-w-md overflow-hidden flex flex-col"
                        style={{ boxShadow: '0 20px 48px rgba(80,40,20,0.18)' }}
                    >
                        <div className="flex items-center justify-between p-6 border-b border-adm-border bg-adm-elev/50">
                            <h3 id="modal-author-title" className="text-lg font-bold text-adm-ink">{editingIndex !== null ? 'Editar Autor' : 'Novo Autor'}</h3>
                            <button onClick={() => setIsModalOpen(false)} aria-label="Fechar modal" className="w-10 h-10 min-h-[44px] min-w-[44px] flex items-center justify-center text-adm-ink-faint hover:text-adm-ink hover:bg-adm-elev rounded transition-colors"><X className="w-4 h-4" aria-hidden="true" /></button>
                        </div>
                        <div className="p-6 overflow-y-auto max-h-[70vh] flex flex-col gap-6">
                            <label aria-label="Foto do autor — clique para fazer upload" className="w-28 h-28 rounded-full overflow-hidden border-4 border-adm-border shadow-inner bg-adm-elev flex flex-col items-center justify-center mx-auto relative group cursor-pointer">
                                ...
                                {tempAuthor.avatar ? (
                                    ...
                                    <div className="flex flex-col items-center text-adm-ink-faint group-hover:text-adm-primary transition-colors">
                                ...
                                {[
                                    ...
                                    <div key={f.key}>
                                        ...
                                        <input type={f.type} placeholder={f.placeholder} value={tempAuthor[f.key] || ''} onChange={e => setTempAuthor({ ...tempAuthor, [f.key]: e.target.value })}
                                            className="w-full bg-adm-elev border border-adm-border rounded-md px-4 py-3 text-sm text-adm-ink focus:outline-none focus:ring-2 focus:ring-adm-primary/30 font-bold text-center" />
                                    </div>
                                ))}
                                <div>
                                    ...
                                    <textarea rows={4} placeholder="Escreva sobre as especialidades do autor..." value={tempAuthor.bio || ''} onChange={e => setTempAuthor({ ...tempAuthor, bio: e.target.value })}
                                        className="w-full bg-adm-elev border border-adm-border rounded-md px-4 py-3 text-sm text-adm-ink-muted focus:outline-none focus:ring-2 focus:ring-adm-primary/30 resize-none text-center leading-relaxed" />
                                </div>
                                <div className="pt-4 border-t border-adm-border">
                                    <p className="text-xs font-black text-adm-ink-faint mb-3 uppercase tracking-widest text-center">Redes Sociais</p>
                                    <div className="space-y-3">
                                        {[
                                            ...
                                            <div key={f.key}>
                                                <label className="block text-[10px] font-bold text-adm-ink-faint mb-1 uppercase tracking-wider">{f.label}</label>
                                                <input
                                                    type="url"
                                                    ...
                                                    className="w-full bg-adm-elev border border-adm-border rounded-lg px-3 py-2 text-xs text-adm-ink focus:outline-none focus:ring-2 focus:ring-adm-primary/30"
                                                />
                                            </div>
                                        ))}
                                    </div>
                        ...
                        <div className="p-6 border-t border-adm-border bg-adm-elev flex gap-3 justify-end rounded-b-lg">
                            <button onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 min-h-[44px] text-sm font-semibold text-adm-ink-muted hover:bg-adm-surface rounded transition-colors">Cancelar</button>
                            <button onClick={saveModalAuthor} className="px-6 py-2.5 min-h-[44px] text-sm font-semibold bg-adm-primary hover:brightness-90 text-adm-surface rounded flex items-center gap-2 transition-all">
