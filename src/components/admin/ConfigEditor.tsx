...
export default function ConfigEditor() {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-20 text-adm-ink-faint bg-adm-surface rounded-3xl border border-adm-border">
            <Loader2 className="w-8 h-8 animate-spin mb-4 text-adm-primary" />
    ...
    const inputClass = "w-full bg-adm-surface border border-adm-border rounded-md px-4 py-3 text-sm focus:outline-none focus:border-adm-primary/80 focus:ring-2 focus:ring-adm-primary/20/20 transition-all shadow-sm text-adm-ink font-medium";
    ...
    return (
        <form onSubmit={handleSave} className="space-y-8 pb-32 max-w-3xl">
            {/* Action Bar */}
            <div className="flex items-center justify-between bg-adm-surface p-4 px-6 rounded-lg border border-adm-border shadow-sm">
                <div>
                    <h2 className="text-lg font-bold text-adm-ink">Configurações Gerais</h2>
                    <p className="text-xs text-adm-ink-muted mt-0.5">Edita o arquivo <code className="bg-adm-elev px-1 rounded">src/data/siteConfig.json</code></p>
                </div>
                <button type="submit" disabled={saving} className="bg-adm-primary hover:bg-adm-primary disabled:bg-slate-300 disabled:cursor-not-allowed text-white px-5 py-2.5 rounded-lg font-bold flex items-center gap-2 shadow-sm shadow-none/20 transition-all">
            ...
            {/* Identidade */}
            <div className="p-8 bg-adm-surface border border-adm-border rounded-lg shadow-sm">
                <h3 className="text-xl font-bold text-adm-ink mb-8 border-b border-adm-border pb-4">Identidade Base</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="md:col-span-2 flex flex-col sm:flex-row gap-8 items-start">
                        <div className="w-full sm:w-1/3">
                            ...
                            <label aria-label="Enviar logo do site" className="group relative border-2 border-dashed border-adm-border hover:border-adm-primary/80 bg-adm-elev hover:bg-adm-primary-soft/50 rounded-lg p-6 flex flex-col items-center justify-center cursor-pointer transition-all text-center h-48">
                                ...
                                {config?.logo ? (
                                    ...
                                    <div className="w-16 h-16 bg-adm-surface rounded-full flex items-center justify-center text-adm-ink-faint shadow-sm mb-3 group-hover:text-adm-primary transition-colors">
                                        <ImageIcon className="w-8 h-8" />
                                    </div>
                                )}
                                <span className="text-sm font-semibold text-adm-ink group-hover:text-adm-primary transition-colors">
                            ...
                            <div className="mt-3">
                                <label className="text-xs font-semibold text-adm-ink-muted flex justify-between mb-1">
                                    ...
                                    <span className="font-mono text-adm-primary">{config?.logoHeight ?? 40}px</span>
                                </label>
                                <input type="range" min={24} max={120} step={2} value={config?.logoHeight ?? 40} onChange={e => setConfig({ ...config, logoHeight: Number(e.target.value) })} className="w-full accent-adm-primary" />
                            </div>
                        </div>
                        <div className="w-full sm:w-1/3">
                            <label className={labelClass}>Favicon</label>
                            <label aria-label="Enviar favicon do site" className="group relative border-2 border-dashed border-adm-border hover:border-adm-primary/80 bg-adm-elev hover:bg-adm-primary-soft/50 rounded-lg p-6 flex flex-col items-center justify-center cursor-pointer transition-all text-center h-48">
                                ...
                                {config?.favicon ? (
                                    ...
                                    <div className="w-12 h-12 bg-adm-surface rounded-lg flex items-center justify-center text-adm-ink-faint shadow-sm mb-3 group-hover:text-adm-primary transition-colors text-2xl">⭐</div>
                                )}
                                <span className="text-sm font-semibold text-adm-ink group-hover:text-adm-primary transition-colors">
                                    {config?.favicon ? 'Trocar Favicon' : 'Enviar Favicon'}
                                </span>
                                <span className="text-[10px] text-adm-ink-faint mt-1">PNG, SVG ou ICO</span>
                            </label>
                        </div>
                    </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {presetThemes.map(preset => (
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-3 py-2 bg-adm-elev border border-adm-border rounded-md hover:border-adm-primary/60 hover:bg-adm-primary-soft transition-all text-sm font-semibold text-adm-ink"
                                    >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            {[
                                ...
                                <div key={f.key}>
                                    ...
                                    <div className="flex gap-4 p-2 bg-adm-elev border border-adm-border rounded-md">
                                        ...
                                        <input type="text" value={config?.theme?.[f.key] || ''} onChange={e => setConfig({ ...config, theme: { ...config.theme, [f.key]: e.target.value } })} className="flex-1 bg-transparent border-none focus:outline-none font-mono text-adm-ink font-bold" />
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <div className="p-8 bg-adm-surface border border-adm-border rounded-lg shadow-sm">
                <h3 className="text-xl font-bold text-adm-ink mb-2 border-b border-adm-border pb-4">Estrutura da URL dos posts</h3>
                <p className="text-xs text-adm-ink-muted mb-6 leading-relaxed">
                    Quando você muda essa opção, as URLs antigas ganham redirect 301 automático no <code className="bg-adm-elev px-1 rounded">vercel.json</code>. SEO preservado.
                </p>
                <div className="flex gap-2">
                    {[
                        ...
                        return (
                            <label key={opt.value} className={`relative flex-1 p-4 border-2 rounded-md cursor-pointer transition-all text-center ${active ? 'border-adm-primary/80 bg-adm-primary-soft shadow-sm' : 'border-adm-border hover:border-adm-border'}`}>
                                ...
                                {opt.recommended && (
                                    <span className="absolute -top-2 left-1/2 -translate-x-1/2 text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-adm-primary text-white">recomendado</span>
                                )}
                                <p className="text-sm font-mono font-bold text-adm-ink mt-1">{opt.label}</p>
                                <p className="text-[11px] text-adm-ink-muted mt-1">{opt.desc}</p>
                            </label>
                        );
                    })}
                </div>
            ...
            {/* Exibição dos Posts */}
            <div className="p-8 bg-adm-surface border border-adm-border rounded-lg shadow-sm">
                <h3 className="text-xl font-bold text-adm-ink mb-8 border-b border-adm-border pb-4">Exibição dos Posts</h3>
                <label className="flex items-center justify-between gap-4 p-4 bg-adm-elev border border-adm-border rounded-md cursor-pointer hover:border-adm-primary/60 transition-all">
                    <div>
                    ...
                    <input type="checkbox" checked={!!config?.hidePostDate} onChange={e => setConfig({ ...config, hidePostDate: e.target.checked })} className="w-5 h-5 accent-adm-primary" />
                </label>
            </div>

            {/* Informações de Contato */}
            <div className="p-8 bg-adm-surface border border-adm-border rounded-lg shadow-sm">
                <h3 className="text-xl font-bold text-adm-ink mb-8 border-b border-adm-border pb-4">Informações de Contato</h3>
            ...
            {/* Redes Sociais */}
            <div className="p-8 bg-adm-surface border border-adm-border rounded-lg shadow-sm">
                <h3 className="text-xl font-bold text-adm-ink mb-8 border-b border-adm-border pb-4">Redes Sociais (Rodapé)</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="p-8 bg-adm-surface border border-adm-border rounded-lg shadow-sm">
                <h3 className="text-xl font-bold text-adm-ink mb-2 border-b border-adm-border pb-4">Galeria do Instagram (rodapé do site)</h3>
                <p className="text-sm text-adm-ink-muted mb-6">Cole até 6 URLs de imagens (ex: capturas dos seus posts no Instagram). Se vazio, mostra placeholders. Cada imagem linka pro perfil definido em Redes Sociais &gt; instagram.</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Array.from({ length: 6 }).map((_, i) => {
                        ...
                        return (
                            <div key={i} className="bg-adm-elev border border-adm-border rounded-lg p-3 space-y-2">
                                <div className="flex items-center gap-3">
                                    {src ? (
                                        <img src={src} alt={`Insta ${i+1}`} className="w-14 h-14 rounded-md object-cover border border-adm-border" />
                                    ) : (
                                        <div className="w-14 h-14 rounded-md bg-adm-elev flex items-center justify-center text-adm-ink-faint text-xs">{i+1}</div>
                                    )}
            ...
            {/* Rodape (Footer) */}
            <div className="p-8 bg-adm-surface border border-adm-border rounded-lg shadow-sm">
                <h3 className="text-xl font-bold text-adm-ink mb-8 border-b border-adm-border pb-4">Rodape (Footer)</h3>
            ...
            {/* SEO Global */}
            <div className="p-8 bg-adm-surface border border-adm-border rounded-lg shadow-sm">
                <h3 className="text-xl font-bold text-adm-ink mb-8 border-b border-adm-border pb-4">SEO Global</h3>
                <div className="space-y-4">
            ...
            {/* Sitemap */}
            <div className="p-8 bg-adm-surface border border-adm-border rounded-lg shadow-sm">
                <h3 className="text-xl font-bold text-adm-ink mb-8 border-b border-adm-border pb-4">Sitemap</h3>
                <div className="space-y-4">
                    <div className="bg-emerald-50 border border-emerald-200 rounded-md p-4">
                        ...
                        {config?.url ? (
                            <div className="space-y-2">
                                <a href={`${config.url.replace(/\/$/, '')}/sitemap-index.xml`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm font-bold text-emerald-700 bg-adm-surface px-4 py-2 rounded-lg border border-emerald-200 hover:bg-emerald-100 transition-colors">
            ...
            {/* Robots.txt */}
            <div className="p-8 bg-adm-surface border border-adm-border rounded-lg shadow-sm">
                <h3 className="text-xl font-bold text-adm-ink mb-2 flex items-center gap-2">🤖 Robots.txt</h3>
                <p className="text-sm text-adm-ink-muted mb-6 border-b border-adm-border pb-4">
                    Controla quais páginas os buscadores (Google, Bing) podem indexar. <code className="bg-adm-elev px-1 rounded">/admin</code> e <code className="bg-adm-elev px-1 rounded">/api</code> já são bloqueados por padrão. O sitemap é linkado automaticamente.
                </p>
                <div className="space-y-4">
                    <div className="flex items-center justify-between p-4 bg-adm-elev border border-adm-border rounded-md">
                        <div>
                            <label className="text-sm font-bold text-adm-ink">Bloquear todos os buscadores</label>
                        ...
                        <button
                            ...
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors flex-shrink-0 ml-4 focus:outline-none focus:ring-2 focus:ring-adm-primary/30 focus:ring-offset-2 ${config?.robots?.noindex ? 'bg-red-600' : 'bg-adm-elev border border-adm-border'}`}
                        >
                            <span aria-hidden="true" className={`inline-block h-4 w-4 transform rounded-full bg-adm-surface shadow transition-transform ${config?.robots?.noindex ? 'translate-x-6' : 'translate-x-1'}`} />
                        </button>
                    </div>
                    ...
                    {config?.url ? (
                        <div className="bg-adm-elev border border-adm-border rounded-md p-4">
                            <p className="text-xs font-bold text-adm-ink-muted mb-2">Preview / Verificar:</p>
                            <a href={`${config.url.replace(/\/$/, '')}/robots.txt`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm font-bold text-adm-ink hover:text-adm-primary transition-colors">
                                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
