...
export default function PostsManager() {
    ...
    return (
        <div className="space-y-6 pb-32">
            ...
            {/* Header */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-adm-surface p-5 px-6 rounded-lg border border-adm-border shadow-sm sticky top-0 z-40">
                <div>
                ...
                <a href="/admin/posts/new" className="bg-adm-primary hover:bg-adm-primary text-white px-6 py-3 rounded-md font-bold flex items-center gap-2 shadow-lg shadow-none/25 hover:-translate-y-0.5 transition-all">
            ...
            {/* Filters */}
            <div className="bg-adm-surface p-4 rounded-lg border border-adm-border shadow-sm flex flex-wrap gap-3 items-center">
                <div className="relative flex-1 min-w-[200px]">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-adm-ink-faint" />
                    <input type="text" aria-label="Buscar artigos" placeholder="Buscar artigos..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 bg-adm-elev border border-adm-border rounded-md text-sm focus:outline-none focus:border-adm-primary/80 focus:ring-1 focus:ring-adm-primary/20" />
                </div>
                <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="bg-adm-elev border border-adm-border rounded-md px-3 py-2.5 text-sm text-adm-ink focus:outline-none focus:border-adm-primary/80">
                    <option value="all">Todos</option>
                    <option value="published">Publicados</option>
                    <option value="draft">Rascunhos</option>
                </select>
                {dynamicCategories.length > 0 && (
                    <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className="bg-adm-elev border border-adm-border rounded-md px-3 py-2.5 text-sm text-adm-ink focus:outline-none focus:border-adm-primary/80">
            ...
            {/* Table */}
            {paginated.length === 0 ? (
                <div className="bg-adm-surface border-2 border-dashed border-adm-border rounded-lg p-16 flex flex-col items-center justify-center text-center">
                    <FileText className="w-10 h-10 text-adm-ink-faint mb-4" aria-hidden="true" />
                    {search || statusFilter !== 'all' || categoryFilter !== 'all' ? (
                        <>
                            <h3 className="text-base font-semibold text-adm-ink mb-2">Nenhum artigo encontrado</h3>
                            <p className="text-sm text-adm-ink-muted mb-5">Tente ajustar os filtros ou a busca.</p>
                            <button
                                onClick={() => { setSearch(''); setStatusFilter('all'); setCategoryFilter('all'); }}
                                className="text-sm font-semibold text-adm-primary hover:underline"
                            >
                                Limpar filtros
                            </button>
                        </>
                        ...
                        <>
                            <h3 className="text-base font-semibold text-adm-ink mb-2">Nenhum artigo ainda</h3>
                            <p className="text-sm text-adm-ink-muted mb-5 max-w-xs">Escreva o primeiro artigo para começar a publicar conteúdo no seu blog.</p>
                            <a href="/admin/posts/new" className="inline-flex items-center gap-2 bg-adm-primary hover:brightness-90 text-adm-surface font-semibold px-6 py-2.5 rounded text-sm transition-all">
                ...
                <div className="bg-adm-surface rounded-lg border border-adm-border overflow-hidden shadow-sm">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="border-b border-adm-border bg-adm-elev">
                                <tr>
                                    ...
                                    <th scope="col" aria-sort={sortField === 'title' ? (sortOrder === 'asc' ? 'ascending' : 'descending') : 'none'} className="py-4 px-4 text-xs font-bold text-adm-ink-muted uppercase tracking-wider cursor-pointer hover:text-adm-ink" onClick={() => toggleSort('title')}>Título <SortIcon field="title" /></th>
                                    <th scope="col" className="py-4 px-4 text-xs font-bold text-adm-ink-muted uppercase tracking-wider hidden sm:table-cell">Categoria</th>
                                    <th scope="col" className="py-4 px-4 text-xs font-bold text-adm-ink-muted uppercase tracking-wider hidden md:table-cell">Autor</th>
                                    <th scope="col" aria-sort={sortField === 'pubDate' ? (sortOrder === 'asc' ? 'ascending' : 'descending') : 'none'} className="py-4 px-4 text-xs font-bold text-adm-ink-muted uppercase tracking-wider cursor-pointer hover:text-adm-ink hidden md:table-cell" onClick={() => toggleSort('pubDate')}>Data <SortIcon field="pubDate" /></th>
                                    <th scope="col" className="py-4 px-4 text-xs font-bold text-adm-ink-muted uppercase tracking-wider">Status</th>
                                    <th scope="col" className="py-4 px-4 text-xs font-bold text-adm-ink-muted uppercase tracking-wider text-right">Ações</th>
                                </tr>
                            </thead>
                                ...
                                {paginated.map(post => (
                                    <React.Fragment key={post.sha}>
                                        <tr className="hover:bg-adm-elev transition-colors">
                                            ...
                                            <td className="py-4 px-4 hidden sm:table-cell"><span className="text-xs font-bold bg-adm-elev text-adm-ink-muted px-2 py-1 rounded-full">{post.category}</span></td>
                                                ...
                                                <div className="flex items-center justify-end gap-1">
                                                    {!post.draft && (
                                                        <a
                                                            ...
                                                            title="Abrir no site"
                                                            className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-adm-ink-faint hover:text-adm-primary hover:bg-adm-primary-soft rounded transition-colors"
                                                        >
                                                    ...
                                                    <a
                                                        ...
                                                        className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-adm-ink-faint hover:text-adm-ink-muted hover:bg-adm-elev rounded transition-colors"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        title="Excluir"
                                                        className="p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-adm-ink-faint hover:text-red-600 hover:bg-red-50 rounded transition-colors"
                                                    >
                                                ...
                                                <td colSpan={7} className="px-4 py-4">
                                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                                        <div>
                                                            ...
                                                            <input type="text" value={quickEditData.title} onChange={e => setQuickEditData({ ...quickEditData, title: e.target.value })} className="w-full bg-adm-surface border border-adm-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-adm-primary/80" />
                                                        </div>
                                                        <div>
                                                            <label className="block text-[10px] font-bold text-adm-ink-muted uppercase tracking-wider mb-1">URL do artigo</label>
                                                            <input type="text" value={quickEditData.slug} onChange={e => setQuickEditData({ ...quickEditData, slug: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-') })} className="w-full bg-adm-surface border border-adm-border rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:border-adm-primary/80" />
                                                        </div>
                                                        <div>
                                                            <label className="block text-[10px] font-bold text-adm-ink-muted uppercase tracking-wider mb-1">Data</label>
                                                            <input type="date" value={quickEditData.pubDate} onChange={e => setQuickEditData({ ...quickEditData, pubDate: e.target.value })} className="w-full bg-adm-surface border border-adm-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-adm-primary/80" />
                                                        </div>
                                                        <div>
                                                            <label className="block text-[10px] font-bold text-adm-ink-muted uppercase tracking-wider mb-1">Categoria</label>
                                                            <input type="text" list="cats-list" value={quickEditData.category} onChange={e => setQuickEditData({ ...quickEditData, category: e.target.value })} className="w-full bg-adm-surface border border-adm-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-adm-primary/80" />
                                                        ...
                                                        <div>
                                                            ...
                                                            <input type="text" list="authors-list" value={quickEditData.author} onChange={e => setQuickEditData({ ...quickEditData, author: e.target.value })} className="w-full bg-adm-surface border border-adm-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-adm-primary/80" />
                                                    ...
                                                    <div className="flex gap-3 mt-4 justify-end">
                                                        <button onClick={() => { setEditingSha(null); setQuickEditError(''); }} className="flex items-center gap-1.5 px-4 py-2 text-sm font-bold text-adm-ink-muted hover:bg-adm-elev rounded-lg transition-colors"><X className="w-4 h-4" aria-hidden="true" />Cancelar</button>
                                                        <button onClick={saveQuickEdit} disabled={saving} className="flex items-center gap-1.5 px-5 py-2 text-sm font-bold bg-adm-primary hover:bg-adm-primary text-white rounded-lg shadow-sm transition-all disabled:opacity-60">
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-center gap-2">
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                        <button key={p} onClick={() => setCurrentPage(p)} aria-label={`Página ${p}`} aria-current={currentPage === p ? 'page' : undefined} className={`w-11 h-11 rounded text-sm font-semibold transition-all ${currentPage === p ? 'bg-adm-primary text-adm-surface' : 'bg-adm-surface border border-adm-border text-adm-ink-muted hover:border-adm-primary/40'}`}>{p}</button>
                    ))}
                </div>
            )}
        </div>
