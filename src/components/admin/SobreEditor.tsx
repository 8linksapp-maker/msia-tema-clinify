...
export default function SobreEditor() {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-32 text-adm-ink-faint bg-adm-surface rounded-lg border border-adm-border">
            <LayoutTemplate className="w-10 h-10 animate-pulse mb-6 text-adm-ink-faint" />
            <p className="font-semibold text-sm animate-pulse text-adm-ink-muted">Buscando sobre.json...</p>
        </div>
    );

    const cardClass = "p-8 mb-6 bg-adm-surface border border-adm-border rounded-lg shadow-sm";
    ...
    return (
        <div className="max-w-4xl space-y-0 pb-32">
            ...
            <div className="flex items-center justify-between bg-adm-surface p-4 px-6 rounded-lg border border-adm-border shadow-sm mb-6">
                <div>
                    <h2 className="text-lg font-bold text-adm-ink">Editar Página: Sobre Nós</h2>
                    <p className="text-xs text-adm-ink-muted mt-0.5">Edita o arquivo <code className="bg-adm-elev px-1 rounded">src/data/sobre.json</code></p>
                </div>
                <button onClick={handleSave} disabled={saving} className="bg-adm-primary hover:bg-adm-primary disabled:opacity-50 text-white px-5 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all">
            ...
            <form onSubmit={handleSave} className="space-y-6">
                <div className={cardClass}>
                    <h3 className="text-lg font-bold text-adm-ink mb-6 border-b border-adm-border pb-4">1. Banner da História (Hero)</h3>
                    <div className="space-y-4">
                        ...
                        <div className="bg-adm-elev p-4 border border-adm-border rounded-md">
                            <label className={labelClass}>Foto da Empresa</label>
                            <input type="file" accept="image/*" aria-label="Selecionar foto da empresa ou equipe" className="text-sm file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:bg-adm-primary-soft file:text-adm-primary hover:file:bg-adm-primary-soft cursor-pointer" onChange={e => handleFileSelect(e, 'heroImg')} />
                            {pendingUploads['heroImg'] && <span className="text-[10px] bg-adm-elev text-adm-ink px-2 py-1 rounded font-bold uppercase mt-2 inline-block">Upload Pendente</span>}
                            {sobre?.hero?.image && <div className="mt-4 w-full h-[200px] border border-adm-border rounded overflow-hidden"><img src={sobre?.hero?.image} className="w-full h-full object-cover" /></div>}
                        </div>
                    </div>
                </div>

                <div className={cardClass}>
                    <h3 className="text-lg font-bold text-adm-ink mb-6 border-b border-adm-border pb-4">2. Conteúdo da História</h3>
                    <div className="space-y-4">
                ...
                <div className={cardClass}>
                    <h3 className="text-lg font-bold text-adm-ink mb-6 border-b border-adm-border pb-4">SEO</h3>
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="file" accept="image/*" aria-label="Selecionar imagem para compartilhamento social (Open Graph)" onChange={e => handleFileSelect(e, 'seoImg')} className="text-[10px] w-full file:mr-2 file:py-1 file:px-2 file:border-0 file:bg-adm-primary-soft file:text-adm-primary" />
