...
export default function BackupManager({ siteName }: { siteName: string }) {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      {/* Header */}
      <div className="bg-adm-surface border border-adm-border rounded-lg p-6 shadow-sm">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-adm-ink tracking-tight" style={{ letterSpacing: '-0.02em' }}>
              Backup de posts
            </h1>
            <p className="text-sm text-adm-ink-muted mt-1 leading-relaxed">
              Migre posts entre sites MSIA. O arquivo <code className="bg-adm-elev px-1.5 py-0.5 rounded text-xs">.zip</code> inclui todos os posts (markdown) + imagens referenciadas neles. Sem produtos, sem landings, sem configuração — só conteúdo editorial.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-adm-surface border border-adm-border rounded-lg p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h2 className="font-bold text-adm-ink">Exportar</h2>
          </div>
          <p className="text-sm text-adm-ink-muted mb-5 leading-relaxed">
            Baixa um <code className="bg-adm-elev px-1 rounded text-xs">.zip</code> com todos os posts deste site + imagens.
          </p>
          <button
          ...
          <div className="mt-4 p-3 bg-adm-elev border border-adm-border rounded-lg text-[11px] text-adm-ink-muted leading-relaxed">
        ...
        <div className="bg-adm-surface border border-adm-border rounded-lg p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h2 className="font-bold text-adm-ink">Importar</h2>
          </div>
          <p className="text-sm text-adm-ink-muted mb-5 leading-relaxed">
          ...
          {!selectedFile ? (
            ...
            <div className="flex items-center justify-between gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="font-mono text-xs text-adm-ink truncate">{selectedFile.name}</span>
                <span className="text-[10px] text-adm-ink-faint shrink-0">({fmtBytes(selectedFile.size)})</span>
              </div>
              <button onClick={reset} className="text-adm-ink-faint hover:text-red-600 shrink-0">
      ...
      {(previewing || preview) && (
        <div className="bg-adm-surface border border-adm-border rounded-lg p-6 shadow-sm">
          {previewing ? (
            <div className="flex items-center justify-center py-12 text-adm-ink-muted gap-3">
            ...
            <>
              <div className="flex items-start justify-between mb-4 flex-wrap gap-3">
                <div>
                  <h2 className="text-lg font-bold text-adm-ink">
                    {preview.applied ? 'Importação concluída' : 'Preview do import'}
                  </h2>
                  {preview.manifest && (
                    <p className="text-xs text-adm-ink-muted mt-1 font-mono">
                      origem: <span className="text-adm-ink">{preview.manifest.source_site || '?'}</span>
                      {' · '}exportado em: <span className="text-adm-ink">{preview.manifest.exported_at?.slice(0, 10) || '?'}</span>
                    </p>
                  )}
                </div>
                <div className="flex items-center gap-4 text-sm">
                  <span><strong className="text-adm-ink">{preview.posts.length}</strong> <span className="text-adm-ink-muted">posts</span></span>
                  <span><strong className="text-adm-ink">{preview.images.length}</strong> <span className="text-adm-ink-muted">imagens</span></span>
                  <span className="text-adm-ink-faint">{fmtBytes(preview.total_size)}</span>
                </div>
              </div>
                        ...
                        <button
                          ...
                          className={`px-3 py-2.5 min-h-[44px] text-xs font-bold rounded-lg transition-colors ${
                            conflictPolicy === 'skip'
                              ...
                              : 'bg-adm-surface text-adm-ink border border-adm-border hover:border-amber-300'
                          }`}
                        >
                        ...
                        <button
                          ...
                          className={`px-3 py-2.5 min-h-[44px] text-xs font-bold rounded-lg transition-colors ${
                            conflictPolicy === 'overwrite'
                              ...
                              : 'bg-adm-surface text-adm-ink border border-adm-border hover:border-red-300'
                          }`}
                        >
                          Sobrescrever todos
                        </button>
              ...
              {!preview.applied && (
                <div className="mt-6 flex items-center justify-end gap-3">
                  <button onClick={reset} className="px-4 py-2.5 min-h-[44px] text-sm font-bold text-adm-ink-muted hover:text-adm-ink">
                    Cancelar
                  </button>
                  <button
...
function FileTable({ title, icon: Icon, items }: { title: string; icon: React.ElementType; items: PreviewItem[] }) {
  if (items.length === 0) {
    return (
      <div className="bg-adm-elev border border-adm-border rounded-md p-4 text-center text-sm text-adm-ink-faint">
        <Icon className="w-5 h-5 inline-block mr-2 opacity-50" /> Sem {title.toLowerCase()}
      </div>
    );
  }
  return (
    <div className="bg-adm-elev border border-adm-border rounded-md overflow-hidden">
      <div className="px-4 py-2.5 min-h-[44px] bg-adm-elev border-b border-adm-border flex items-center gap-2">
        <Icon className="w-3.5 h-3.5 text-adm-ink-muted" />
        <span className="text-xs font-bold text-adm-ink uppercase tracking-wider">{title}</span>
        <span className="text-xs text-adm-ink-faint ml-auto">{items.length}</span>
      </div>
      <ul className="max-h-72 overflow-y-auto divide-y divide-slate-200">
        {items.map((item) => (
          <li key={item.name} className="px-4 py-2.5 min-h-[44px] text-xs flex items-center gap-2">
            ...
            <span className="font-mono text-adm-ink truncate flex-1" title={item.name}>{item.name}</span>
            <span className="text-adm-ink-faint shrink-0 text-[10px]">{fmtBytes(item.size)}</span>
          </li>
        ))}
      </ul>
    </div>
...
function StatusBadge({ item }: { item: PreviewItem }) {
  ...
  if (item.status === 'skipped') return <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-adm-elev text-adm-ink-muted shrink-0">pulado</span>;
