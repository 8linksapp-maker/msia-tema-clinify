...
export default function PostEditor({ filePath }: PostEditorProps) {
    ...
    const labelClass = "block text-xs font-semibold text-adm-ink-muted uppercase tracking-widest mb-2";
        ...
        <div className="max-w-5xl pb-32">
            ...
            <div className="flex items-center justify-between bg-adm-surface p-4 px-6 rounded-lg border border-adm-border shadow-sm mb-6">
                <div className="flex items-center gap-3">
                    <a href="/admin/posts" aria-label="Voltar para lista de artigos" className="text-adm-ink-faint hover:text-adm-primary transition-colors p-2 min-h-[44px] min-w-[44px] flex items-center justify-center rounded hover:bg-adm-primary-soft"><ArrowLeft className="w-5 h-5" aria-hidden="true" /></a>
                    <div>
                        <p className="text-xs font-semibold text-adm-ink-faint uppercase tracking-widest">{isEditing ? 'Editar artigo' : 'Novo artigo'}</p>
                        <p className="text-sm font-semibold text-adm-ink line-clamp-1 mt-0.5">
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="flex items-center gap-1.5 px-4 py-2.5 min-h-[44px] bg-adm-surface border border-adm-border hover:bg-adm-elev text-adm-ink-muted hover:text-adm-ink rounded text-sm font-medium transition-colors"
                    >
                    ...
                    <button
                        onClick={handleSave}
                        disabled={saving}
                        className="bg-adm-primary hover:brightness-90 disabled:opacity-50 disabled:cursor-not-allowed text-adm-surface px-5 py-2.5 min-h-[44px] rounded text-sm font-semibold flex items-center gap-2 transition-all"
            ...
            <div className="flex gap-6 items-start">
                ...
                <div className="flex-1 min-w-0 space-y-6">
                    ...
                    <div className="bg-adm-surface p-6 rounded-lg border border-adm-border shadow-sm">
                        ...
                        <input
                            ...
                            className="w-full bg-transparent border-0 border-b border-transparent hover:border-adm-border focus:border-adm-primary/80 focus:outline-none font-display font-normal text-3xl md:text-4xl text-adm-ink leading-tight tracking-tight pb-3 transition-colors"
                        />

                        {/* Permalink — estilo WordPress: "Endereço: /slug-do-post  Editar" */}
                        {post.title && (
                            <div className="mt-3 flex items-center flex-wrap gap-2 text-sm text-adm-ink-muted">
                                <span className="font-semibold text-adm-ink-faint uppercase tracking-widest text-xs">Endereço:</span>
                                {editingSlug ? (
                                    <div className="flex items-center gap-2 flex-1 min-w-0">
                                        <span className="font-mono text-adm-ink-faint shrink-0">/</span>
                                        <input
                                            type="text"
                                            value={post.slug}
                                            onChange={e => setPost(p => ({ ...p, slug: slugify(e.target.value) }))}
                                            aria-label="URL do artigo"
                                            className="flex-1 bg-adm-elev border border-adm-border rounded px-2 py-1 text-sm font-mono text-adm-ink focus:outline-none focus:border-adm-primary/80 min-w-0"
                                            autoFocus
                                        />
                                        <button
                                            type="button"
                                            onClick={() => setEditingSlug(false)}
                                            className="px-3 py-1 text-xs font-semibold text-adm-primary hover:text-adm-ink transition-colors"
                                        >
                                            OK
                                        </button>
                                    </div>
                                    ...
                                    <>
                                        <span className="font-mono text-adm-ink truncate">/{post.slug || 'sem-endereco'}</span>
                                        <button
                                            type="button"
                                            onClick={() => setEditingSlug(true)}
                                            className="px-2 py-1 text-xs font-semibold text-adm-primary hover:text-adm-ink transition-colors"
                                        >
                                            Editar
                                        </button>
                                    </>
                        ...
                        {/* Descrição / Resumo */}
                        <div className="mt-5 pt-5 border-t border-adm-border">
                    ...
                    <div className="bg-adm-surface p-6 rounded-lg border border-adm-border shadow-sm">
                        ...
                        {showVideoBar && !isPreview && (
                            <div className="mb-3 flex items-center gap-2 p-3 bg-adm-elev rounded-md border border-adm-border">
                                <Video className="w-4 h-4 text-adm-ink-faint shrink-0" aria-hidden="true" />
                                <div className="flex-1 min-w-0">
                                    <input
                                        ...
                                        className="w-full bg-adm-surface border border-adm-border rounded px-3 py-1.5 text-sm font-mono focus:outline-none focus:border-adm-primary/80 transition-colors"
                                        autoFocus
                                ...
                                <button
                                    ...
                                    className="px-3 py-1.5 min-h-[36px] bg-adm-primary text-adm-surface text-xs font-semibold rounded hover:brightness-90 disabled:opacity-40 disabled:cursor-not-allowed transition-all shrink-0"
                                >
                                    Inserir
                                </button>
                                <button
                                    ...
                                    className="text-adm-ink-faint hover:text-adm-ink transition-colors shrink-0 p-1"
                                >
                                    ×
                                </button>
                            </div>
                        ...
                        {isPreview ? (
                            <div className="ql-editor border border-adm-border rounded-md" style={{ minHeight: '480px', background: 'rgb(255 254 251)' }} dangerouslySetInnerHTML={{ __html: post.content }} />
                        ) : QuillEditor ? (
                            <QuillEditor
                                ref={quillRef}
                                theme="snow"
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-adm-surface border border-adm-border rounded-md px-4 py-3 text-sm text-adm-ink font-mono focus:outline-none focus:border-adm-primary/80 resize-y"
                                    style={{ minHeight: '300px' }}
                                    aria-label="Conteúdo do artigo"
                                />
                            </div>
                        ) : (
                            <div className="flex items-center justify-center p-12 text-adm-ink-faint">
                ...
                <div className="w-72 shrink-0 space-y-4 sticky top-4">
                    {/* Publish Settings */}
                    <div className="bg-adm-surface p-5 rounded-lg border border-adm-border shadow-sm">
                        <h3 className="font-bold text-adm-ink text-sm border-b border-adm-border pb-3 mb-4">Publicação</h3>
                        <div className="space-y-4">
                            <div>
                                <label className={labelClass}>Status</label>
                                <label className="flex items-center gap-3 cursor-pointer p-3 bg-adm-elev rounded-md hover:bg-adm-primary-soft transition-colors">
                                    <input type="checkbox" checked={post.draft} onChange={e => setPost(p => ({ ...p, draft: e.target.checked }))} className="rounded border-adm-border text-adm-primary focus:ring-adm-primary/20" />
                                    <span className="text-sm font-medium text-adm-ink">Salvar como rascunho</span>
                                </label>
                            </div>
                            <div>
                    ...
                    {/* Category & Author */}
                    <div className="bg-adm-surface p-5 rounded-lg border border-adm-border shadow-sm">
                        <h3 className="font-bold text-adm-ink text-sm border-b border-adm-border pb-3 mb-4">Metadados</h3>
                        <div className="space-y-4">
                            <div>
                    ...
                    {/* Hero Image */}
                    <div className="bg-adm-surface p-5 rounded-lg border border-adm-border shadow-sm">
                        <h3 className="font-bold text-adm-ink text-sm border-b border-adm-border pb-3 mb-4">Imagem de Capa</h3>
                        <label className="group relative border-2 border-dashed border-adm-border hover:border-adm-primary/60 bg-adm-elev hover:bg-adm-primary-soft rounded-md flex flex-col items-center justify-center cursor-pointer transition-all text-center overflow-hidden" style={{ minHeight: '120px' }}>
                            ...
                            {post.heroImage ? (
                                <>
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-adm-surface/20">
                                        <ImageIcon className="w-8 h-8 text-adm-ink" />
                                        <span className="text-xs font-bold text-adm-ink mt-1">Trocar imagem</span>
                                    </div>
                                </>
                            ) : (
                                <div className="py-6 flex flex-col items-center text-adm-ink-faint group-hover:text-adm-primary transition-colors">
