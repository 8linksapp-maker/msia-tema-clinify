...
const BADGE_STYLES: Record<string, { bg: string; text: string; icon: string }> = {
  ...
  'Recomendado':             { bg: 'bg-adm-primary-soft',  text: 'text-violet-800', icon: '⭐' },
  "Editor's Choice":         { bg: 'bg-rose-100',    text: 'text-rose-800',   icon: '✍️' },
  'Premium':                 { bg: 'bg-purple-100',  text: 'text-purple-800', icon: '💎' },
  'Orçamento':               { bg: 'bg-adm-elev',   text: 'text-adm-ink',  icon: '💰' },
};
...
function StarRating({ rating }: { rating: number }) {
  ...
  return (
    <div className="flex items-center gap-0.5">
      ...
      <span className="ml-1 text-adm-ink-muted text-xs font-bold tabular-nums">
  ...
  if (!hasContent) {
    return (
      <div className="flex flex-col items-center justify-center h-56 text-center px-4">
        ...
        <p className="text-sm font-semibold text-adm-ink-faint">Preencha os dados</p>
        <p className="text-xs text-adm-ink-faint mt-1">A prévia do card aparece aqui</p>
      </div>
    );
  }

  return (
    <div className="rounded-md border border-adm-border overflow-hidden bg-adm-surface shadow-sm">
      ...
      <div className="flex gap-3 p-3">
        {form.image && (
          <div className="w-16 h-16 shrink-0 rounded-lg bg-adm-elev border border-adm-border overflow-hidden">
            <img
              src={form.image}
              alt=""
        ...
        <div className="flex-1 min-w-0">
          <p className="font-bold text-adm-ink leading-tight mb-1 line-clamp-2" style={{ fontSize: '12px' }}>
            {form.title || <span className="text-adm-ink-faint font-normal">Título do produto...</span>}
          </p>
          ...
          {form.price && (
            <div className="flex items-baseline gap-1.5 mt-1">
              ...
              {form.originalPrice && (
                <span className="text-adm-ink-faint line-through" style={{ fontSize: '10px' }}>{form.originalPrice}</span>
              )}
            </div>
          )}
        </div>
      ...
      {(pros.length > 0 || cons.length > 0) && (
        <div className="px-3 pb-2 grid grid-cols-2 gap-x-2">
          <div className="space-y-0.5">
            {pros.map((p, i) => (
              <div key={i} className="flex items-start gap-1 text-adm-ink-muted" style={{ fontSize: '11px' }}>
...
const inputClass =
  'w-full bg-adm-surface border border-adm-border rounded-md px-4 py-3 text-sm font-medium text-adm-ink focus:outline-none focus:border-adm-primary/80 focus:ring-2 focus:ring-adm-primary/20/20 transition-all shadow-sm';
const labelClass = 'block text-xs font-bold text-adm-ink-muted uppercase tracking-wider mb-2 ml-0.5';
  ...
  return (
    <div className="max-w-4xl space-y-5">
      ...
      >
        <div className="flex items-start justify-between mb-5">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <div className="w-9 h-9 bg-adm-surface/20 rounded-md flex items-center justify-center backdrop-blur-sm border border-white/30">
          ...
          <div
            className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border ${
              config.enabled
                ? 'bg-adm-surface/15 border-white/30 text-white'
                : 'bg-black/20 border-black/20 text-white/50'
            }`}
          >
            <div className={`w-1.5 h-1.5 rounded-full ${config.enabled ? 'bg-green-300 animate-pulse' : 'bg-adm-surface/30'}`} />
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          {[
            ...
            <div
              key={i}
              className="flex items-center gap-2.5 bg-adm-surface/10 backdrop-blur-sm rounded-md px-4 py-2.5 border border-white/20"
            >
              {stat.icon}
              <div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-1 bg-adm-elev p-1 rounded-md">
          <button
            onClick={() => setTab('products')}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${
              tab === 'products' ? 'bg-adm-surface text-adm-ink shadow-sm' : 'text-adm-ink-muted hover:text-adm-ink'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${
              tab === 'settings' ? 'bg-adm-surface text-adm-ink shadow-sm' : 'text-adm-ink-muted hover:text-adm-ink'
            }`}
          >
      ...
      {tab === 'products' && (
        <>
          ...
          {showForm && (
            <div className="bg-adm-surface rounded-lg border border-adm-border shadow-sm">
              {/* Form header */}
              <div className="px-6 py-4 border-b border-adm-border flex items-center justify-between bg-gradient-to-r from-slate-50 to-amber-50/40 rounded-t-2xl">
                <div>
                ...
                <button
                  ...
                  className="text-xs text-adm-ink-faint hover:text-adm-ink-muted bg-adm-elev hover:bg-adm-elev px-3 py-1.5 rounded-lg font-medium transition-colors"
                >
                  Cancelar
                </button>
              </div>
              ...
              <div className="grid lg:grid-cols-[1fr_300px] divide-y lg:divide-y-0 lg:divide-x divide-adm-border">
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="flex gap-2 p-3 bg-amber-50 border border-amber-200 rounded-md">
                    <div className="flex-1">
                      <input
                        ...
                        className="w-full bg-adm-surface border border-amber-200 rounded-lg px-3 py-2 text-sm font-mono text-adm-ink focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-400/20 transition-all"
                  ...
                  <div className="grid grid-cols-[1fr_auto] gap-3 items-end">
                    ...
                    {form.image && (
                      <img
                        src={form.image}
                        alt="preview"
                        className="w-14 h-14 object-contain rounded-md border border-adm-border bg-adm-elev shadow-sm"
                  ...
                  <div>
                    <label className={labelClass}>
                      URL principal <span className="text-adm-ink-faint font-normal normal-case tracking-normal">— Amazon ou outra loja (opcional se houver lojas adicionais abaixo)</span>
                    </label>
                    <input
                      type="url"
                  ...
                  <div className="border-t border-adm-border pt-4">
                    ...
                    {extraLinks.length === 0 ? (
                      <p className="text-xs text-adm-ink-faint">Mercado Livre, Magalu, Shopee...</p>
                    ) : (
                      <div className="space-y-3">
                        {extraLinks.map((link, i) => (
                          <div key={i} className="bg-adm-surface border border-adm-border rounded-lg p-3">
                            <div className="flex items-center gap-2 mb-2">
                              <label className="text-xs font-bold text-adm-ink-muted flex-1">Loja {i + 1}</label>
                              <button
                                type="button"
                                onClick={() => setExtraLinks(ls => ls.filter((_, j) => j !== i))}
                                className="p-1 text-adm-ink-faint hover:text-red-500 rounded transition-colors"
                              >
                  ...
                  <label className="flex items-center gap-3 cursor-pointer p-3 bg-adm-elev rounded-md hover:bg-amber-50 transition-colors w-fit">
                    <input
                      type="checkbox"
                      checked={form.enabled}
                      onChange={e => setForm(f => ({ ...f, enabled: e.target.checked }))}
                      className="rounded border-adm-border text-amber-500 focus:ring-amber-400"
                    />
                    <span className="text-sm font-medium text-adm-ink">Produto ativo (visível nos posts)</span>
                  </label>

                  <div className="flex gap-3 pt-2 border-t border-adm-border">
                    <button
                    ...
                    <button
                      onClick={() => { setShowForm(false); setEditingId(null); }}
                      className="px-5 py-2.5 rounded-md text-sm font-bold text-adm-ink-muted bg-adm-elev hover:bg-adm-elev transition-all"
                    >
                      Cancelar
                    </button>
                  </div>
                ...
                <div className="p-5 bg-gradient-to-b from-slate-50/80 to-slate-100/40 rounded-br-2xl">
                  <div className="sticky top-6">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <p className="text-xs font-bold text-adm-ink-muted uppercase tracking-wider">Prévia ao vivo</p>
                    </div>
                    ...
                    {(form.slug || form.title) && (
                      <div className="mt-4 p-3 bg-amber-50 border border-amber-100 rounded-md">
                        ...
                        <div className="flex items-center gap-2">
                          <code className="text-xs bg-adm-surface border border-amber-200 rounded-lg px-2 py-1.5 font-mono text-amber-800 flex-1 truncate">
            ...
            <div className="p-4 bg-red-50 border border-red-200 rounded-lg flex items-center justify-between gap-4">
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-adm-elev hover:bg-adm-border text-adm-ink-muted text-sm font-bold rounded-md transition-colors"
                >
                  Cancelar
                </button>
              </div>
          ...
          {!showForm && products.length === 0 && (
            <div className="bg-adm-surface rounded-lg border border-dashed border-adm-border p-12 text-center">
              ...
              <h3 className="font-bold text-adm-ink mb-4 text-lg">Nenhum produto ainda</h3>
              <div className="flex items-start justify-center gap-2 mb-6 max-w-sm mx-auto">
                {[
                  ...
                  <div key={i} className="flex-1 text-center">
                    ...
                    <p className="text-xs text-adm-ink-faint leading-snug">{step}</p>
                  </div>
                ))}
              </div>
              <button
            ...
            <div className="space-y-2.5">
              {products.map(p => {
                ...
                return (
                  <div
                    key={p.id}
                    className={`bg-adm-surface rounded-lg border transition-all group ${
                      !p.enabled
                        ? 'border-adm-border opacity-60'
                        : 'border-adm-border hover:border-amber-200 hover:shadow-sm hover:shadow-amber-50'
                    }`}
                  >
                    <div className="flex items-center gap-4 p-4">
                      {/* Image */}
                      <div className="w-16 h-16 rounded-md border border-adm-border bg-adm-elev flex items-center justify-center overflow-hidden shrink-0">
                        {p.image ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-0.5">
                          <p className="font-bold text-adm-ink leading-tight">{p.title}</p>
                          {p.badge && badgeInfo && (
                          ...
                          {!p.enabled && (
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-adm-elev text-adm-ink-faint">
                              Inativo
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-3 flex-wrap mt-1">
                          ...
                          {p.originalPrice && (
                            <span className="text-xs text-adm-ink-faint line-through">{p.originalPrice}</span>
                          )}
                          {p.extraLinks && p.extraLinks.length > 0 && (
                            <span className="text-xs text-adm-ink-faint font-medium">
                      ...
                      <div className="flex items-center gap-0.5 shrink-0">
                        <button
                          ...
                          title="Editar"
                          className="p-2 text-adm-ink-faint hover:text-adm-primary hover:bg-adm-primary-soft rounded-md transition-colors"
                        >
                        ...
                        >
                          {p.enabled
                            ...
                            : <ToggleLeft className="w-5 h-5 text-adm-ink-faint" />
                          }
                        </button>
                        <button
                          onClick={() => handleDelete(p.id)}
                          title="Remover"
                          className="p-2 text-adm-ink-faint hover:text-red-500 hover:bg-red-50 rounded-md transition-colors"
                        >
            ...
            <div className="rounded-lg border border-amber-100 p-5 bg-gradient-to-br from-amber-50 to-orange-50/60">
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div
                    key={item.code}
                    className="flex items-start gap-3 bg-adm-surface/70 rounded-md p-3 border border-amber-100"
                  >
                    <code className="text-xs bg-adm-surface border border-amber-200 rounded-lg px-2 py-1 font-mono text-amber-800 shrink-0 shadow-sm whitespace-nowrap">
                      {item.code}
                    </code>
      ...
      {/* ── SETTINGS TAB ── */}
      {tab === 'settings' && (
        <div className="bg-adm-surface rounded-lg border border-adm-border shadow-sm p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              />
              <p className="text-xs text-adm-ink-faint mt-1.5 ml-1">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                className="w-12 h-12 rounded-md border border-adm-border cursor-pointer p-1 shadow-sm"
              />
              <input
                type="text"
                value={config.buttonColor}
          ...
          <div className="border border-adm-border rounded-lg overflow-hidden">
            <button
              type="button"
              onClick={() => setShowApiCredentials(v => !v)}
              className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-adm-elev transition-colors"
            >
              <div className="flex items-center gap-2">
                <KeyRound className="w-4 h-4 text-amber-500" />
                <span className="text-sm font-bold text-adm-ink">Importação automática por ASIN (avançado — opcional)</span>
              </div>
              <ChevronDown className={`w-4 h-4 text-adm-ink-faint transition-transform ${showApiCredentials ? 'rotate-180' : ''}`} />
            </button>
            {showApiCredentials && (
              <div className="px-5 pb-5 space-y-4 border-t border-adm-border">
                <p className="text-xs text-adm-ink-faint mt-3 leading-relaxed">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <button
                        ...
                        className="absolute right-3 top-1/2 -translate-y-1/2 text-adm-ink-faint hover:text-adm-ink-muted"
                      >
                ...
                <p className="text-xs text-adm-ink-faint ml-0.5">
          ...
          <div className="border-t border-adm-border pt-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {([
                ...
                <label
                  key={key}
                  className="flex items-center gap-3 cursor-pointer p-3 bg-adm-elev rounded-md hover:bg-amber-50 transition-colors"
                >
                  <input
                    type="checkbox"
                    checked={!!config[key]}
                    ...
                    className="rounded border-adm-border text-amber-500 focus:ring-amber-400"
                  />
                  <span className="text-sm font-medium text-adm-ink">{label}</span>
                </label>
              ))}
            </div>
          </div>
