    ...
    return (
        <div className="max-w-2xl space-y-6">
            ...
            <div className="bg-adm-surface rounded-lg border border-adm-border shadow-sm p-6">
                ...
                <div
                    className={`border-2 border-dashed rounded-md p-8 text-center cursor-pointer transition-colors ${file ? 'border-adm-primary/40 bg-adm-primary-soft' : 'border-adm-border hover:border-adm-primary/40 hover:bg-adm-primary-soft/50'}`}
                    onClick={() => fileInputRef.current?.click()}
                >
                    {file ? (
                        <>
                            <FileText className="w-8 h-8 text-adm-primary mx-auto mb-2" />
                        ...
                        <>
                            <Upload className="w-8 h-8 text-adm-ink-faint mx-auto mb-2" />
            ...
            {previewLoading && (
                <div className="bg-adm-primary-soft border border-adm-primary/30 rounded-md p-4 flex items-center gap-3">
                    <Loader2 className="w-4 h-4 animate-spin text-adm-primary shrink-0" />
                    <p className="text-sm text-adm-primary font-medium">Lendo arquivo...</p>
                </div>
            )}
            {preview && !previewLoading && !result && (
                <div className="bg-adm-surface rounded-lg border border-adm-primary/30 shadow-sm p-5">
                    <p className="text-sm font-bold text-adm-ink mb-3">Encontrado no arquivo</p>
                    <div className="grid grid-cols-3 gap-3 mb-4">
                        {[
                            ...
                            <div key={s.label} className="bg-adm-elev rounded-md p-3 text-center">
                                <p className="text-2xl font-bold text-adm-primary">{s.value}</p>
                                <p className="text-xs text-adm-ink-muted">{s.label}</p>
                            </div>
                        ))}
                    </div>
                    <p className="text-xs text-adm-ink-faint">Posts com o mesmo slug já existentes serão ignorados. Pronto para importar.</p>
                </div>
            )}
            ...
            <button
                ...
                className="bg-adm-primary hover:bg-adm-primary disabled:opacity-50 disabled:cursor-not-allowed text-white px-6 py-3 rounded-md text-sm font-bold flex items-center gap-2 transition-all shadow-sm shadow-none/20"
            >
                {importing ? (
            ...
            {importing && (
                <div className="bg-adm-primary-soft border border-adm-primary/30 rounded-md p-4">
            ...
            {/* Resultado */}
            {result && (
                <div className={`bg-adm-surface rounded-lg border shadow-sm p-6 ${result.success ? 'border-green-200' : 'border-amber-200'}`}>
