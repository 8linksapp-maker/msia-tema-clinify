...
function VariablesPanel({ bodyRef, onInsert }: {
    ...
    return (
        <div className="border border-adm-border rounded-md overflow-hidden">
            <button
                type="button"
                onClick={() => setOpen(v => !v)}
                className="w-full flex items-center justify-between px-3 py-2 bg-adm-elev text-xs font-semibold text-adm-ink-muted hover:bg-adm-primary-soft transition-colors"
            >
                Variáveis disponíveis
                {open ? <ChevronUp className="w-3.5 h-3.5" aria-hidden="true" /> : <ChevronDown className="w-3.5 h-3.5" aria-hidden="true" />}
            </button>
            {open && (
                <div className="p-3 space-y-2 bg-adm-surface">
                    {VARIABLES.map(v => (
                        <div key={v.token} className="flex items-center justify-between gap-3">
                            <div>
                                <code className="text-xs font-mono bg-adm-elev px-1.5 py-0.5 rounded text-adm-primary">{v.token}</code>
                                <span className="text-xs text-adm-ink-muted ml-2">{v.label}</span>
                            </div>
                            <button
                                type="button"
                                onClick={() => onInsert(v.token)}
                                className="text-xs font-medium text-adm-primary hover:underline shrink-0"
                            >
                                Inserir
                            </button>
                        </div>
...
export default function EmailSequenceEditor() {
    ...
    const inputClass = 'w-full bg-adm-surface border border-adm-border rounded-md px-4 py-3 text-sm font-medium text-adm-ink focus:outline-none focus:border-adm-primary/80 focus:ring-2 focus:ring-adm-primary/20/20 transition-all';
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center p-16 text-adm-ink-faint">
            <Loader2 className="w-7 h-7 animate-spin mb-3 text-adm-primary" />
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-adm-primary-soft border border-adm-primary/30 rounded-lg p-4 flex gap-3">
                <Clock className="w-5 h-5 text-adm-primary shrink-0 mt-0.5" />
                <div>
            ...
            <div className="bg-adm-surface rounded-lg border border-adm-border shadow-sm p-5">
                <label className="block text-sm font-bold text-adm-ink mb-2">Email para testes</label>
                <input
                    type="email"
                    value={testEmail}
            ...
            {/* Lista de emails */}
            {emails.length === 0 ? (
                <div className="text-center py-12 text-adm-ink-faint bg-adm-surface rounded-lg border border-dashed border-adm-border">
                ...
                <div className="space-y-4">
                    {emails.map((emailItem, idx) => {
                        ...
                        return (
                        <div key={emailItem.id} className="bg-adm-surface rounded-lg border border-adm-border shadow-sm p-5">
                            {/* Header do email */}
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-2 flex-wrap">
                                    <span className="w-7 h-7 bg-adm-primary-soft text-adm-primary rounded-full flex items-center justify-center text-xs font-bold">{idx + 1}</span>
                                    <span className="text-sm font-bold text-adm-ink">Email #{idx + 1}</span>
                                    {stat && (
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center gap-3 pt-1">
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-3 py-2 border border-adm-border rounded-md text-xs font-semibold text-adm-ink hover:bg-adm-elev disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                                    >
            ...
            <div className="flex gap-3">
                <button
                    type="button"
                    onClick={addEmail}
                    className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-adm-border rounded-md text-sm font-medium text-adm-ink-muted hover:border-adm-primary/60 hover:text-adm-primary hover:bg-adm-primary-soft transition-all"
                >
