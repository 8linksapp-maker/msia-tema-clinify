...
const sourceBadge: Record<string, string> = {
    popup: 'bg-adm-primary-soft text-adm-primary',
    widget: 'bg-blue-100 text-blue-700',
    api: 'bg-adm-elev text-adm-ink-muted',
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    <div className="w-9 h-9 bg-adm-primary-soft rounded-md flex items-center justify-center">
                ...
                <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-adm-surface border border-adm-border rounded-md text-sm font-medium text-adm-ink hover:bg-adm-elev disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm"
                >
            ...
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full pl-9 pr-4 py-2.5 bg-adm-surface border border-adm-border rounded-md text-sm focus:outline-none focus:border-adm-primary/80 focus:ring-2 focus:ring-adm-primary/20/20 transition-all"
                />
            </div>
                    ...
                    {subscribers.length === 0 ? (
                        <>
                            ...
                            <a
                                ...
                                className="inline-block px-4 py-2 bg-adm-primary text-white rounded-md text-xs font-semibold hover:opacity-90 transition-opacity"
                            >
                ...
                <>
                    <div className="bg-adm-surface rounded-lg border border-adm-border overflow-hidden shadow-sm">
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-adm-border bg-adm-elev">
                            ...
                            <tbody>
                                {paginated.map((sub, i) => (
                                    <tr key={sub.email} className={`border-b border-slate-50 hover:bg-adm-elev/50 transition-colors ${i === paginated.length - 1 ? 'border-0' : ''}`}>
                                        <td className="px-4 py-3 font-medium text-adm-ink">{sub.email}</td>
                                        <td className="px-4 py-3 text-adm-ink-muted">{sub.name || <span className="text-adm-ink-faint">—</span>}</td>
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${sourceBadge[sub.source] || 'bg-adm-elev text-adm-ink-muted'}`}>
                        ...
                        <div className="flex items-center justify-between text-sm">
                            ...
                            <div className="flex items-center gap-1">
                                <button
                                    ...
                                    disabled={safePage === 1}
                                    className="p-1.5 rounded-md border border-adm-border text-adm-ink-muted hover:bg-adm-elev disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                                >
                                    <ChevronLeft className="w-4 h-4" aria-hidden="true" />
                                </button>
                                <span className="px-3 py-1 text-xs font-medium text-adm-ink-muted">
                                    {safePage} / {totalPages}
                                </span>
                                <button
