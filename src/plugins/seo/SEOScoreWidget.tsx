  ...
  return (
    <div className="bg-adm-surface p-5 rounded-lg border border-adm-border shadow-sm">
      <div className="flex items-center justify-between border-b border-adm-border pb-3 mb-4">
        <h3 className="font-bold text-adm-ink text-sm">Checklist básico de SEO</h3>
        {hasTitle && <span className={`text-lg font-black ${scoreText}`}>{score}%</span>}
      </div>

      {!hasTitle ? (
        <p className="text-xs text-adm-ink-faint text-center py-3">
          Preencha o título e a descrição para ver sua pontuação.
        </p>
      ) : (
        <>
          {/* Progress bar */}
          <div className="h-2 bg-adm-elev rounded-full mb-4 overflow-hidden">
            <div
            ...
            {checks.map(check => (
              <div key={check.label} className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <p className={`text-xs font-semibold ${check.pass ? 'text-adm-ink' : 'text-adm-ink-muted'}`}>
                    {check.label}
                  </p>
          ...
          <p className="text-xs text-adm-ink-faint mt-4 pt-3 border-t border-adm-border leading-relaxed">
