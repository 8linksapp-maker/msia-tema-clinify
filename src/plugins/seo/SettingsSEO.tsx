...
export default function SettingsSEO() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="bg-adm-surface rounded-lg border border-adm-border shadow-sm p-6">
        <label className="flex items-center justify-between cursor-pointer">
          <div>
            ...
            <p className="text-sm text-adm-ink-muted mt-0.5">Ajuda o Google a entender seu site e pode mostrar informações extras nos resultados de busca.</p>
          </div>
          <div
            onClick={() => setEnabled(!enabled)}
            className={`relative w-12 h-6 rounded-full transition-colors cursor-pointer ${enabled ? 'bg-adm-primary' : 'bg-adm-elev'}`}
          >
      ...
      {/* Organization */}
      <div className="bg-adm-surface rounded-lg border border-adm-border shadow-sm p-6 space-y-4">
        ...
        <div>
          ...
          <p className="text-xs text-adm-ink-faint mb-3">Adicione os perfis da sua marca nas redes sociais</p>
      ...
      <div className="bg-adm-surface rounded-lg border border-adm-border shadow-sm p-6">
        ...
        <div className="space-y-3">
          {[
            ...
            <label key={label} className="flex items-center justify-between p-3 rounded-md bg-adm-elev cursor-pointer hover:bg-adm-primary-soft transition-colors">
              <div>
                <p className="text-sm font-semibold text-adm-ink">{label}</p>
                <p className="text-xs text-adm-ink-muted">{desc}</p>
              </div>
              <input type="checkbox" checked={val} onChange={e => set(e.target.checked)} className="rounded border-adm-border text-adm-primary focus:ring-adm-primary/20 w-4 h-4" />
            </label>
          ))}
        </div>
      </div>
      ...
      {/* Status pós-save */}
      {savedStatus && (
        <div className="bg-adm-surface border border-adm-border rounded-lg p-4 flex items-center gap-4">
          {savedStatus.orgLogo ? (
            <img
              src={savedStatus.orgLogo}
              alt="Logo da organização"
              className="w-10 h-10 rounded-md object-contain border border-adm-border bg-adm-elev shrink-0"
              onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }}
            />
          ) : (
            <div className="w-10 h-10 rounded-md bg-adm-elev border border-adm-border flex items-center justify-center shrink-0">
              <Building2 className="w-5 h-5 text-adm-ink-faint" aria-hidden="true" />
            </div>
          )}
          <div>
            <p className="font-semibold text-adm-ink text-sm">{savedStatus.orgName || 'Organização sem nome'}</p>
