...
export default function SettingsSocialShare() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      {/* Enable toggle */}
      <div className="bg-adm-surface rounded-lg border border-adm-border shadow-sm p-6">
        <label className="flex items-center justify-between cursor-pointer">
          <div>
            <h3 className="font-bold text-adm-ink">Ativar Compartilhamento</h3>
            <p className="text-sm text-adm-ink-muted mt-0.5">Os botões aparecem ao final de cada artigo do blog.</p>
          </div>
          <div
            onClick={() => setEnabled(!enabled)}
            className={`relative w-12 h-6 rounded-full transition-colors ${enabled ? 'bg-adm-primary' : 'bg-adm-elev'}`}
          >
            <div className={`absolute top-1 w-4 h-4 bg-adm-surface rounded-full shadow transition-all ${enabled ? 'left-7' : 'left-1'}`} />
          </div>
        </label>
      </div>
      ...
      <div className="bg-adm-surface rounded-lg border border-adm-border shadow-sm p-6">
        ...
        {sectionTitle && (
          <p className="text-xs text-adm-ink-faint mt-2 ml-1">
            Preview: <span className="text-xs font-bold uppercase tracking-widest text-adm-ink-muted">{sectionTitle}</span>
          </p>
        )}
      </div>
      ...
      <div className="bg-adm-surface rounded-lg border border-adm-border shadow-sm p-6">
        ...
        <div className="grid grid-cols-2 gap-2">
          {PLATFORMS.map(p => (
            <label key={p.id} className={`flex items-center gap-3 p-3 rounded-md border cursor-pointer transition-all ${platforms.includes(p.id) ? 'border-adm-primary/40 bg-adm-primary-soft' : 'border-adm-border hover:bg-adm-elev'}`}>
              <input
                type="checkbox"
                checked={platforms.includes(p.id)}
                onChange={() => togglePlatform(p.id)}
                className="rounded border-adm-border text-adm-primary focus:ring-adm-primary/20"
              />
              <span className="text-lg">{p.icon}</span>
              <span className="text-sm font-medium text-adm-ink">{p.label}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      <div className="bg-adm-surface rounded-lg border border-adm-border shadow-sm p-6">
        ...
        <div className="mt-5 space-y-3">
          <p className="text-xs font-bold text-adm-ink-muted uppercase tracking-wider">Exemplos de estilo</p>
          {/* Apenas ícones */}
          <div className={`p-3 rounded-md border transition-all ${style === 'icon' ? 'border-adm-primary/40 bg-adm-primary-soft' : 'border-adm-border bg-adm-elev'}`}>
            <p className="text-xs text-adm-ink-faint mb-2">Apenas ícones</p>
            <div className="flex gap-2 flex-wrap">
              {PLATFORMS.filter(p => platforms.includes(p.id)).slice(0, 5).map(p => (
                <span key={p.id} className="w-9 h-9 flex items-center justify-center rounded-md bg-adm-surface border border-adm-border text-base">{p.icon}</span>
              ))}
            </div>
          </div>
          {/* Apenas texto */}
          <div className={`p-3 rounded-md border transition-all ${style === 'label' ? 'border-adm-primary/40 bg-adm-primary-soft' : 'border-adm-border bg-adm-elev'}`}>
            <p className="text-xs text-adm-ink-faint mb-2">Apenas texto</p>
            <div className="flex gap-2 flex-wrap">
              {PLATFORMS.filter(p => platforms.includes(p.id)).slice(0, 5).map(p => (
                <span key={p.id} className="px-3 py-1.5 rounded-md bg-adm-surface border border-adm-border text-xs font-semibold text-adm-ink">{p.label}</span>
              ))}
            </div>
          </div>
          {/* Ícone + texto */}
          <div className={`p-3 rounded-md border transition-all ${style === 'both' ? 'border-adm-primary/40 bg-adm-primary-soft' : 'border-adm-border bg-adm-elev'}`}>
            <p className="text-xs text-adm-ink-faint mb-2">Ícone + texto</p>
            <div className="flex gap-2 flex-wrap">
              {PLATFORMS.filter(p => platforms.includes(p.id)).slice(0, 4).map(p => (
                <span key={p.id} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-adm-surface border border-adm-border text-xs font-semibold text-adm-ink">
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className="bg-adm-primary hover:bg-adm-primary disabled:opacity-50 text-white px-6 py-3 rounded-md text-sm font-bold flex items-center gap-2 transition-all shadow-sm shadow-none/20"
      >
