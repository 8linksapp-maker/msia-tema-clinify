...
export default function SettingsCookieConsent() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      {/* Texts */}
      <div className={`bg-adm-surface rounded-lg border border-adm-border shadow-sm p-6 space-y-4 ${!enabled ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        <div>
          ...
          <p className="text-xs text-adm-ink-faint mt-1.5 ml-1">Você precisa ter uma página /privacidade no seu site antes de ativar o banner.</p>
        </div>

        {/* Banner preview */}
        <div>
          <p className="text-xs font-bold text-adm-ink-muted uppercase tracking-wider mb-2 ml-1">Preview do banner</p>
          <div className="rounded-md bg-gray-900 px-5 py-4 flex flex-col gap-3">
            ...
            <div className="flex gap-2">
              <span className="bg-adm-primary text-white text-xs font-bold px-3 py-1.5 rounded">
      ...
      {/* Position */}
      <div className={`bg-adm-surface rounded-lg border border-adm-border shadow-sm p-6 ${!enabled ? 'opacity-50 pointer-events-none' : ''}`}>
        <h3 className="font-bold text-adm-ink mb-4">Posição do Banner</h3>
        <div className="flex gap-3">
          {(['bottom', 'top'] as const).map(pos => (
            <label key={pos} className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-md border cursor-pointer transition-all ${position === pos ? 'border-adm-primary/40 bg-adm-primary-soft text-adm-primary' : 'border-adm-border text-adm-ink-muted hover:bg-adm-elev'}`}>
