...
export default function SettingsAI() {
    ...
    return (
        <div className="max-w-2xl space-y-6">
            ...
            {/* Indicador de passos */}
            <div className="bg-adm-surface rounded-lg border border-adm-border shadow-sm p-4">
                <p className="text-xs font-bold text-adm-ink-faint uppercase tracking-widest mb-3">Como configurar</p>
                <ol className="flex flex-wrap gap-x-4 gap-y-2">
                    {STEP_LABELS.map((label, i) => (
                        <li key={i} className="flex items-center gap-1.5 text-xs text-adm-ink-muted">
                            <span className="w-5 h-5 rounded-full bg-adm-elev text-adm-ink-faint flex items-center justify-center text-xs font-bold shrink-0">{i + 1}</span>
                            {label}
                        </li>
                    ))}
                </ol>
                ...
                <div className="grid grid-cols-2 gap-3">
                    {PROVIDERS.map(p => (
                        <button
                            ...
                            className={`p-4 rounded-md border-2 text-left transition-all ${provider === p.id ? 'border-adm-primary/80 bg-adm-primary-soft' : 'border-adm-border hover:border-adm-border'}`}
                        >
            ...
            <div className="bg-adm-elev rounded-lg border border-adm-border p-4">
                ...
                <div className="space-y-2 text-sm">
                    {[
                        { label: 'Provedor', value: currentProvider.name, color: 'text-adm-ink font-semibold' },
            ...
            <div className="flex gap-3">
                ...
                <button
                    ...
                    className="bg-adm-primary hover:bg-adm-primary disabled:opacity-50 disabled:cursor-not-allowed text-white px-6 py-2.5 rounded-md text-sm font-bold flex items-center gap-2 transition-all shadow-sm shadow-none/20"
                >
            ...
            {!testedInSession && apiKey.trim() && (
                <p className="text-xs text-adm-ink-faint ml-1">Teste a chave primeiro para habilitar o botão Salvar.</p>
            )}
        </div>
    );
}
