                ...
                <tbody>
                    {rows.map((row, i) => (
                        <tr key={i} className="border-b border-slate-50 hover:bg-adm-elev transition-colors">
                            <td className="py-3 px-4 font-medium text-adm-ink max-w-xs truncate">
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex gap-1 bg-adm-elev rounded-md p-1">
                    {DAYS_OPTIONS.map(opt => (
                        <button
                            key={opt.value}
                            onClick={() => handleDaysChange(opt.value)}
                            className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${days === opt.value ? 'bg-adm-surface text-adm-primary shadow-sm' : 'text-adm-ink-muted hover:text-adm-ink'}`}
                        >
                            {opt.label}
                        </button>
                    ))}
